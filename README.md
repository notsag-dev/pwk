...
https://github.com/notsag-dev/buffer-overflow
...
- `netstat -ano`
- `netsh advfirewall show currentprofile` and `netsh advfirewall firewall show rule name=all`
- Query scheduled tasks: `schtasks /query /fo LIST /v`
- List applications that have been installed using the Windows installer: `wmic product get name, version, vendor`
...
- `route`
- `netstat -ano`
- `/etc/iptables`: Sometimes firewall rules are left here and often have weak permissions
- Search for files generated by `iptables-save` which will be a dump of firewall configs that can be restored using `iptables-restore` (search at /etc)
- Query scheduled tasks (check execution permisions): `ls -lah /etc/cron*`
